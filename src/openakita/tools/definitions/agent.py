...
AGENT_TOOLS = [
    {
        ...
        "description": (
            ...
            "**ASYNC BY DEFAULT**: Delegations run in the background and return immediately "
            "with a delegation_id. After launching, **immediately call check_delegation_status** "
            "to show the user the current status. Use wait_for_delegations to block until "
            "completion, or cancel_delegation to stop. Set run_in_background=false for blocking "
            "mode (waits for result before returning).\n\n"
            "IMPORTANT:\n"
            "- Launch multiple agents concurrently whenever possible for independent tasks\n"
            "- Do NOT launch more than 5 concurrent agents\n"
            "- **After launching async delegations, always check status immediately**\n"
    ...
    },
    {
        ...
        "description": (
            ...
            "specialized for.\n\n"
            "**ASYNC BY DEFAULT**: Returns immediately with a batch_id. **After launching, "
            "immediately call check_delegation_status** to show the user the current status. "
            "Use wait_for_delegations to block until all complete, or cancel_delegation to stop. "
            "Set run_in_background=false for blocking mode."
        ),
        "detail": (
            "Delegate tasks to multiple agents for parallel execution.\n\n"
            "**Async behavior** (default): Returns immediately with a batch_id. "
            "Use check_delegation_status, wait_for_delegations, or cancel_delegation to manage.\n\n"
            "**Core rules**:\n"
