...
def _matches_no_proxy(hostname: str, no_proxy: str) -> bool:
    ...
    for pattern in no_proxy.split(","):
        ...
        if lower == p_lower:
            return True
        # IP 精确匹配（规范化后比较，如 ::1 vs 0:0:0:0:0:0:0:1）
        try:
...
def get_httpx_client_kwargs(
    *, timeout: float = 30.0, is_local: bool = False, target_url: str = "",
) -> dict:
    ...
    Args:
        timeout: 请求超时（秒）
        is_local: 是否为本地端点（向后兼容，推荐改用 target_url）
        target_url: 目标 URL — 自动判断是否跳过代理
    """
    kwargs: dict = {
        "timeout": timeout,
        "trust_env": False,
    ...
    skip_proxy = is_local or (target_url and should_bypass_proxy(target_url))
    if not skip_proxy:
        proxy = get_proxy_config()
        if proxy:
            kwargs["proxy"] = proxy
